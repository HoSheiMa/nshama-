...
<div class="row text-center">
   ...
   <div class="row justify-content-md-center">
    ...
    <h2 class="col-sm-12 col-lg-3 col-xl-3  bg-white" style="padding:8px;">إضافة فرصة تطوعية</h2></div>
   </div>
  ...
  if (in_array($imgtype, $types)) {
    ...
    if ($img['error'] == 0) {
      ...
      if ($img['size'] <= 10000000) {
        ...
        $email = (isset($_SESSION['who']) && isset($_SESSION['log']) && $_SESSION['log'] == true) ? $_SESSION['who'] : $_SESSION['log_admin'];
        $q = "INSERT INTO `volunteer opportunities`(`img`, `id`, `title`, `body`, `date`, `state`,`email`)
   VALUES ('assets/$new_name','$id','$title','$shortbody','$date','1','$email')";
        mysqli_query($c, $q);

        $q = "INSERT INTO  `volunteer opportunities info`(`id`, `team_email`, `state`, `last_time`, `creater_time`, `team_creater`, `person_creater`, `mobile_person_creater`, `type`, `sex`, `location`, `city`, `types_focus`, `item_needed`, `more_body_info`,`time_add`) 
...
<form method='post' style='width:50%;text-center' class='main_form' enctype="multipart/form-data">
  ...
  <div class="form-group " >
    <input type="text" required="required" name="title" class="form-control" id="exampleInputEmail1" aria-describedby="emailHelp" placeholder="العنوان الرئسي ">
  </div>
  <div class="form-group " >
    <input type="text"required="required" name="shortbody"class="form-control" id="exampleInputPassword1" placeholder="تعريف صغير عن ">
  </div>
   <div class="form-group">
  ...
  <div class="form-group">
   ...
   <div class="form-group">
    <input type="shortbody"required="required" name="waytype" class="form-control" id="exampleInputPassword1" placeholder="المسارات التطوعي">
  </div>
