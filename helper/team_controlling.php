...
$q__ = "SELECT * FROM `teams_members` WHERE `team_email`='$email' AND `state`='1'";
            ...
            <div class="col-md-12 col-sm-12">
                <div class="panel panel-default">
                    <div class="panel-heading">
                        <h4 class="panel-title" style="text-align:center;"> اعضاء الفريق </h4>
                    </div>
                            ...
                            <tbody>
                                ...
                                while ($x = mysqli_fetch_array($r__)) {
                                    ...
                                    echo "
                                         <tr>
                                         <th scope=\"row\">
                                         <button class='btn btn-danger' onclick='delete_from_team(\"$x_email\")'>حذف</button>
                                         |
                                         <button class='btn btn-danger' $DISABLED_b onclick='block_from_team(\"$x_email\" ,\"block\")'>حظر</button>
                                         <button class='btn btn-success' $DISABLED_a onclick='block_from_team(\"$x_email\", \"active\")'>تنشيط</button>
                                         </th>
...
function delete_from_team(email) {
    $.ajax({
        url:'../ajax/delete_from_owner_team.php',
        type: 'post',
        data : {
            email: email,
        },
...
function block_from_team(email, state) {
    $.ajax({
        url:'../ajax/block_owner_team.php',
        type: 'post',
        data : {
            email: email,
            state : state,
